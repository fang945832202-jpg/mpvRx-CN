...
object MediaInfoParser {
    ...
    private val SOURCE_TAGS = setOf(
        ...
        "crater", "hbo", "stan", "pmtp", "dsnp", "dnsp",
        "crav", "voot", "zee5", "sonyliv", "jio", "mx",
        "hotstar", "itunes", "vudu", "roku", "tubi"
    )
    ...
    private val MISC_NOISE = setOf(
        "10bit", "8bit", "hdr", "hdr10", "hdr10plus", "dolby",
        "vision", "dovi", "sdr", "bt709", "bt2020", "hlg", "pq",
        "batch", "dvd", "uncensored", "censored",
        "horriblesubs", "subsplease", "hybrid", "open", "matte"
    )
    ...
    fun parse(fileName: String): ParsedMediaInfo {
        ...
        val titleBoundary = findTitleBoundary(
            ...
            seasonWordMatch, epMarkerMatch, ePrefixMatch, yearMatch
        )
    ...
    private fun findYear(
        ...
        val validYears = yearCandidates.filter { yearResult ->
        ...
        if (validYears.size >= 2) {
            ...
            val textBefore = fileName.substring(0, firstYear.range.first)
                .replace(Regex("""[.\s_-]+"""), " ").trim()
